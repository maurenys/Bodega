	...
	$.ajax({
		...
		success : function(response) {
			...
			$("#movimientoTable tbody").on('click','tr',function() {
				$("#idmovim").val($(this).find("td:eq(0)").text());
				$("#idnvent").val($(this).find("td:eq(1)").text());
				$("#tipomovim").val($(this).find("td:eq(0)").text());
				$("#cant_movida").val($(this).find("td:eq(1)").text());
				$("#fecha_movim").val($(this).find("td:eq(0)").text());
				$("#id_rut").val($(this).find("td:eq(1)").text());
				$("#ubic_origen").val($(this).find("td:eq(0)").text());
				$("#ubic_destino").val($(this).find("td:eq(1)").text());
				$("#btn_group_old").show();
				$("#btn_group_new").hide();
				$("#myModal").modal("show");
			});
